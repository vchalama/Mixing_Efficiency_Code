...
REAL*8  DELRHOX2(1:NX,1:NZ,NY_TEMP_MIN:NY_TEMP_MAX),DELRHOZ2(1:NX,1:NZ,NY_TEMP_MIN:NY_TEMP_MAX), &
        DELRHOY2(1:NX,1:NZ,NY_TEMP_MIN:NY_TEMP_MAX), DELRHO2(1:NX,1:NZ,NY_TEMP_MIN:NY_TEMP_MAX), &
        DELRHO2_1D(1:NY_3D_TOT_TEMP),PHI_D,PHI_D_TEMP,PHI_D_NEW,PHI_D_TEMP_NEW,TH_TEMP, &
        KAPPA_T_1D(1:NY_3D_TOT_TEMP),FLUXTERM_X(1:NX,1:NZ,NY_TEMP_MIN:NY_TEMP_MAX),FLUXTERM_Z(1:NX,1:NZ,NY_TEMP_MIN:NY_TEMP_MAX), &
        FLUXTERM_Y(1:NX,1:NZ,NY_TEMP_MIN:NY_TEMP_MAX),FLUXTERM(1:NX,1:NZ,NY_TEMP_MIN:NY_TEMP_MAX), &
        FLUXTERM_1D(1:NY_3D_TOT_TEMP)
!,INDEX_1D(1:NY_3D_TOT_TEMP)

INTEGER  INDEX_1D(1:NY_3D_TOT_TEMP), TEMP6(1:((NY_3D_TOT_TEMP+1)/2))
...
!        ENDDO
        ...
        tmp_index=0 
         ...
         read(1011)(((KAPPA_T_3D_TOT(I,K,J),i=1,NX),k=1,NZ),j=1,NY_TOT)
...
!!!!!!!!!!!!!!!!!!!!!!  CALCULATING THE TERM {DEL. (KAPPA_T DEL RHO)} !!!!!!!!!!!!!
               ...
               IF(I .EQ. 1) THEN
                FLUXTERM_X(I,K,J) = ((NU/PR + KAPPA_T_3D_TOT(I,K,J))*(TH_TOT_3D(I+2,K,J)-2.0*TH_TOT_3D(I+1,K,J)+TH_TOT_3D(I,K,J))/(DX**2.0)) + &
               ...
               ELSEIF(I .EQ. NX) THEN
                FLUXTERM_X(I,K,J) = ((NU/PR + KAPPA_T_3D_TOT(I,K,J))*(TH_TOT_3D(I,K,J)-2.0*TH_TOT_3D(I-1,K,J)+TH_TOT_3D(I-2,K,J))/(DX**2.0)) + &
                                    (((TH_TOT_3D(I,K,J)-TH_TOT_3D(I-1,K,J))/(1.0D0*DX))*((KAPPA_T_3D_TOT(I,K,J)-KAPPA_T_3D_TOT(I-1,K,J))/(1.0D0*DX)))
               ELSEIF(I .EQ. NX-1) THEN
                FLUXTERM_X(I,K,J) = ((NU/PR + KAPPA_T_3D_TOT(I,K,J))*(TH_TOT_3D(I,K,J)-2.0*TH_TOT_3D(I-1,K,J)+TH_TOT_3D(I-2,K,J))/(DX**2.0)) + &
                                    (((TH_TOT_3D(I+1,K,J)-TH_TOT_3D(I-1,K,J))/(2.0D0*DX))*((KAPPA_T_3D_TOT(I+1,K,J)-KAPPA_T_3D_TOT(I-1,K,J))/(2.0D0*DX)))
               ELSE
                FLUXTERM_X(I,K,J) = ((NU/PR + KAPPA_T_3D_TOT(I,K,J))*(TH_TOT_3D(I+1,K,J)-2.0*TH_TOT_3D(I,K,J)+TH_TOT_3D(I-1,K,J))/(1.0D0*(DX**2.0))) + &
                                    (((TH_TOT_3D(I+1,K,J)-TH_TOT_3D(I-1,K,J))/(2.0D0*DX))*((KAPPA_T_3D_TOT(I+1,K,J)-KAPPA_T_3D_TOT(I-1,K,J))/(2.0D0*DX)))
               ENDIF
               ...
               IF(K .EQ. 1) THEN
                FLUXTERM_Z(I,K,J) = ((NU/PR + KAPPA_T_3D_TOT(I,K,J))*(TH_TOT_3D(I,K+2,J)-2.0*TH_TOT_3D(I,K+1,J)+TH_TOT_3D(I,K,J))/(DZ**2.0)) + &
               ...
               ELSEIF(K .EQ. NZ) THEN
                FLUXTERM_Z(I,K,J) = ((NU/PR + KAPPA_T_3D_TOT(I,K,J))*(TH_TOT_3D(I,K,J)-2.0*TH_TOT_3D(I,K-1,J)+TH_TOT_3D(I,K-2,J))/(DZ**2.0)) + &
                                    (((TH_TOT_3D(I,K,J)-TH_TOT_3D(I,K-1,J))/(1.0D0*DZ))*((KAPPA_T_3D_TOT(I,K,J)-KAPPA_T_3D_TOT(I,K-1,J))/(1.0D0*DZ)))
               ELSEIF(K .EQ. NZ-1) THEN
                FLUXTERM_Z(I,K,J) = ((NU/PR + KAPPA_T_3D_TOT(I,K,J))*(TH_TOT_3D(I,K,J)-2.0*TH_TOT_3D(I,K-1,J)+TH_TOT_3D(I,K-2,J))/(DZ**2.0)) + &
                                    (((TH_TOT_3D(I,K+1,J)-TH_TOT_3D(I,K-1,J))/(2.0D0*DZ))*((KAPPA_T_3D_TOT(I,K+1,J)-KAPPA_T_3D_TOT(I,K-1,J))/(2.0D0*DZ)))
               ELSE
                FLUXTERM_Z(I,K,J) = ((NU/PR + KAPPA_T_3D_TOT(I,K,J))*(TH_TOT_3D(I,K+1,J)-2.0*TH_TOT_3D(I,K,J)+TH_TOT_3D(I,K-1,J))/(1.0D0*(DZ**2.0))) + &
                                    (((TH_TOT_3D(I,K+1,J)-TH_TOT_3D(I,K-1,J))/(2.0D0*DZ))*((KAPPA_T_3D_TOT(I,K+1,J)-KAPPA_T_3D_TOT(I,K-1,J))/(2.0D0*DZ)))
               ENDIF


             IF(J .EQ. NY_TEMP_MIN) THEN
                FLUXTERM_Y(I,K,J) = (  (NU/PR + KAPPA_T_3D_TOT(I,K,J))*  &
                                    (((TH_TOT_3D(I,K,J+2)-TH_TOT_3D(I,K,J+1))/(DY(J+1)*DY(J+2))) - ((TH_TOT_3D(I,K,J+1)-TH_TOT_3D(I,K,J))/(DY(J+1)*DY(J+1))))  ) + &
                                    (((TH_TOT_3D(I,K,J+1)-TH_TOT_3D(I,K,J))/(DY(J+1)))*((KAPPA_T_3D_TOT(I,K,J+1)-KAPPA_T_3D_TOT(I,K,J))/(DY(J+1))))
             ELSE IF(J .EQ. NY_TEMP_MIN+1) THEN
                FLUXTERM_Y(I,K,J) = (  (NU/PR + KAPPA_T_3D_TOT(I,K,J))*  &
                                    (((TH_TOT_3D(I,K,J+2)-TH_TOT_3D(I,K,J+1))/(DY(J+1)*DY(J+2))) - ((TH_TOT_3D(I,K,J+1)-TH_TOT_3D(I,K,J))/(DY(J+1)*DY(J+1))))  ) + &
                                    (((TH_TOT_3D(I,K,J+1)-TH_TOT_3D(I,K,J-1))/(GYF(J+1)-GYF(J-1)))*((KAPPA_T_3D_TOT(I,K,J+1)-KAPPA_T_3D_TOT(I,K,J-1))/(GYF(J+1)-GYF(J-1))))
             ELSE IF(J .EQ. NY_TEMP_MAX-1) THEN
                FLUXTERM_Y(I,K,J) = (  (NU/PR + KAPPA_T_3D_TOT(I,K,J))*  &
                                    (((TH_TOT_3D(I,K,J)-TH_TOT_3D(I,K,J-1))/(DY(J)*DY(J))) - ((TH_TOT_3D(I,K,J-1)-TH_TOT_3D(I,K,J-2))/(DY(J)*DY(J-1))))  ) + &
                                    (((TH_TOT_3D(I,K,J+1)-TH_TOT_3D(I,K,J-1))/(GYF(J+1)-GYF(J-1)))*((KAPPA_T_3D_TOT(I,K,J+1)-KAPPA_T_3D_TOT(I,K,J-1))/(GYF(J+1)-GYF(J-1))))
             ELSE IF(J .EQ. NY_TEMP_MAX) THEN
              FLUXTERM_Y(I,K,J) = (  (NU/PR + KAPPA_T_3D_TOT(I,K,J))*    &
                                    (((TH_TOT_3D(I,K,J)-TH_TOT_3D(I,K,J-1))/(DY(J)*DY(J))) - ((TH_TOT_3D(I,K,J-1)-TH_TOT_3D(I,K,J-2))/(DY(J)*DY(J-1))))  ) + &
           ...
           DO j=NY_TEMP_MIN,NY_TEMP_MAX
             ...
             IF(MOD(I,2) .eq. 0) THEN
              DO k=NZ,1,-1
               ...
               KAPPA_T_1D(m) = KAPPA_T_3D_TOT(I,K,J)
...
!          write(6,*) 'MAX MIN VALUES INDEX_1D', MINVAL(INDEX_1D), MAXVAL(INDEX_1D), SIZE(INDEX_1D)
...
!!!!!!!!!!!!!!!!!!  MERGE SORTING   !!!!!!!!!!!!!!!!!!!!!!!!
    ...
    call MergeSort(TH_1D,NY_3D_TOT_TEMP,TEMP1,DV_1D,TEMP2,DELRHO2_1D,TEMP3,KAPPA_T_1D,TEMP4,FLUXTERM_1D,TEMP5,INDEX_1D,TEMP6)
...
!!!!!!!!!!!!!!!!!!! END MERGE SORTING !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      DO i = 1,NY_3D_TOT_TEMP/2
       ...
       tmp4 = KAPPA_T_1D(I)
       KAPPA_T_1D(I)=KAPPA_T_1D(NY_3D_TOT_TEMP-(I-1))
       KAPPA_T_1D(NY_3D_TOT_TEMP-(I-1)) = tmp4

       tmp5 = FLUXTERM_1D(I)
       FLUXTERM_1D(I)=FLUXTERM_1D(NY_3D_TOT_TEMP-(I-1))
       FLUXTERM_1D(NY_3D_TOT_TEMP-(I-1)) = tmp5

       tmp6 = INDEX_1D(I)
       INDEX_1D(I)=INDEX_1D(NY_3D_TOT_TEMP-(I-1))
       INDEX_1D(NY_3D_TOT_TEMP-(I-1)) = tmp6
      ENDDO 
   ...
   call  CALCULATE_Z_AND_ZSTAR
      ...
      i=2
       ...
       DO WHILE ( I .LE. (NY_3D_TOT_TEMP-1)) 
        matchcount=1
...
!        ENDIF
        ...
        KAPPA_T_TEMP = KAPPA_T_1D(I)*DV_1D(I)
...
!!!!!!!!!!! CHECKING IF BOTH VOLUME ELEMENTS HAVE SAME DENSITY !!!!!!!!!
       ...
       IF((I .NE. NY_3D_TOT_TEMP) .AND. (TH_1D(I) .EQ. TH_1D(I+1))) THEN
         ...
         DO WHILE (TH_1D(I) .EQ. TH_1D(I+1))
           ...
           KAPPA_T_TEMP=KAPPA_T_TEMP + KAPPA_T_1D(I+1)*DV_1D(I+1)
         ...
         END DO        
          ...
          DV_TEMP*(DELRHO2_TEMP/DV_TEMP) &
          - (1.d0*(KAPPA_T_TEMP/DV_TEMP)*gravity_g*((Z_STAR_1D(I+1)-ZSTAR_TEMP_2)/(TH_1D(I+1)-TH_TEMP))* &
         DV_TEMP*(DELRHO2_TEMP/DV_TEMP))
       ...
       ELSE
!         TH_MATCHED=.FALSE.
!         write(*,*) 'TH NOT MATCHED',matchcount
          BPE_TEMP = TH_1D(I)*gravity_g*(ZSTAR_TEMP/DV_TEMP)*DV_TEMP
          PHI_D_TEMP =  -(NU/PR)*gravity_g*((Z_STAR_1D(I+1)-Z_PREVIOUS_TEMP)/(TH_1D(I+1)-TH_1D(I-1)))*DV_TEMP*(DELRHO2_TEMP/DV_TEMP) - &
      (1.d0*(KAPPA_T_TEMP/DV_TEMP)*gravity_g*((Z_STAR_1D(I+1)-Z_PREVIOUS_TEMP)/(TH_1D(I+1)-TH_1D(I-1)))*DV_TEMP*(DELRHO2_TEMP/DV_TEMP))
          PHI_D_TEMP_NEW = gravity_g*(ZSTAR_TEMP/DV_TEMP)*(FLUXTERM_TEMP/DV_TEMP)*DV_TEMP
          I = I+1
       ENDIF 
...
!      DO I =1,NX  
!       DIFF_FLUX = DIFF_FLUX + (NU/PR)*gravity_g*gyf(NY_TEMP)*(TH_B(I,NY_TEMP+1)-TH_B(I,NY_TEMP))/(ypoint(I,NY_TEMP+1)-ypoint(I,NY_TEMP))*LX*LZ
!      ENDDO
...
!        IF(Z_STAR_1D(INDEX_2) .LT. 80.0) THEN
!          INDEX_2=INDEX_2+1
!        ELSE
!          exit
!        ENDIF
...
!      I = INDEX_2
!      DISP_SUM_TEMP = 0.D0
!      DISP_TEMP(1:NY_3D_TOT_TEMP) = 0.D0

!      DO WHILE ((Z_STAR_1D(I) .GT. 10.D0))

!          tmp_index = INDEX_1D(I) 
!          DISP_TEMP(I) = Z_STAR_1D(I) - Z_STAR_1D_INITIAL(tmp_index)
!          DISP_SUM_TEMP = DISP_SUM_TEMP + DISP_TEMP(I)*DV_1D(I)
!          IF((DISP_SUM_TEMP .LT. 2.0) .AND. (DISP_SUM_TEMP .GT. -2.0) .AND. (Z_STAR_1D(I) .LT. 60.0d0) ) THEN
!!            write(6,*) 'destination arrived', I, Z_STAR_1D(I),DISP_TEMP(I), DISP_SUM_TEMP
!            INDEX_1 = I
!!            write(6,*) 'TEST INDICES', INDEX_1,INDEX_2
!          ENDIF
!       I = I-1
!      ENDDO
     ...
     IF( (TIME_RATIO .LT. 1.5) .OR. (TIME_RATIO .GT. 2.0)) THEN
     ...
     IF(INDEX_1 .EQ. 1) THEN
      DO
        IF(Z_STAR_1D(INDEX_1) .LT. 15.0) THEN
          INDEX_1=INDEX_1+1
        ELSE
          exit
         ENDIF
     ...
     ELSE         !!!!!!!!!!!!!!!!!!! SECOND INVERSION      
      ...
      DO
        ...
        IF(Z_STAR_1D(INDEX_2) .LT. 45.0) THEN
          INDEX_2=INDEX_2+1
        ELSE
          exit
        ENDIF
     ...
     ENDIF      !!!!!!!!!!!!!!!!!!!!!!!!END OF INVERSION LOOP


     ELSE       !!!!!!!!!!!!!!!!!!!  SECOND PART OF THE CYCLE, ONLY ONE INVERSION  
 
       DO
        IF(Z_STAR_1D(INDEX_1) .LT. 10.0) THEN
          INDEX_1=INDEX_1+1
        ELSE
          exit
         ENDIF
      END DO      
       ...
       DO
        ...
        IF(Z_STAR_1D(INDEX_2) .LT. 30.0) THEN
          INDEX_2=INDEX_2+1
        ELSE
          exit
        ENDIF
...
!     INDEX_2=NY_3D_TOT_TEMP
     ...
     ENDIF
      ...
      DO I=INDEX_1,INDEX_2
         tmp_index = INDEX_1D(I) 
         DISP(I) = Z_STAR_1D(I) - Z_STAR_1D_INITIAL(tmp_index)
...
!         ENDIF
         ...
         L_T(JJ) = L_T(JJ) + ((DISP(I)**2.0)*(DV_1D(I)**1.0))
         N_ZSTAR_AVG(JJ) = N_ZSTAR_AVG(JJ) + N_ZSTAR(I)*DV_1D(I)
         DISP_SUM(JJ) = DISP_SUM(JJ) + DISP(I)*DV_1D(I)


         IF(DV_1D(I) .NE.  DV_INIT(INDEX_1D(I)) ) THEN
          write(6,*) 'VOLUMES NOT MATCHED', I, DV_1D(3) , DV_INIT(INDEX_1D(3)), INDEX_1D(3)
          stop
         ENDIF

        TEMP_SUM = TEMP_SUM + DV_1D(I)
      ENDDO 

!     IF((DISP_SUM(1) .GT. 2.0) .OR.  (DISP_SUM(1) .LT. -2.0)) THEN
!        write(6,*)'DISP SUM NOT LESS THAN 2'
!!        stop
!      ENDIF

      N_ZSTAR_AVG(JJ) = N_ZSTAR_AVG(JJ)/(SUM(DV_1D(INDEX_1:INDEX_2)))
      L_T(JJ) = sqrt(L_T(JJ)/(TEMP_SUM))
      L_O = 0.8*L_T(JJ)
      EPS_INFERRED(JJ) = (L_O**2)*(N_ZSTAR_AVG(JJ)**3.0)
      MIN_DISP=MINVAL(DISP)
      MAX_DISP=MAXVAL(DISP)
 
     ENDDO   !!!!!!!!!!!!!!!   END OF JJ LOOP


!      write(6,*) 'NY_3D_TOT_TEMP', NY_3D_TOT_TEMP
...
!      write(*,*) 'Writing statistics to a file', DBPE_DT,TPE-BPE_TOT,PHI_D
      write(*,*) 'Thrope scale', TIME_RATIO,L_T(1),EPS_INFERRED(1),PHI_D, PHI_D_NEW,DISP_SUM(1),MIN_DISP,MAX_DISP
      write(*,*) 'Averaged stratification',N_ZSTAR_AVG(1),N_ZSTAR_AVG(2)
...
subroutine Merge(A,NA,B,NB,C,NC,D,E,F,X,Y,Z,P,Q,R,S,T,U,A1,B1,C1)

   integer, intent(in) :: NA,NB,NC         ! Normal usage: NA+NB = NC
   real*8, intent(in out) :: A(NA),D(NA),X(NA),P(NA),S(NA)        ! B overlays C(NA+1:NC)
   real*8, intent(in)     :: B(NB),E(NB),Y(NB),Q(NB),T(NB)
   real*8, intent(in out) :: C(NC),F(NC),Z(NC),R(NC),U(NC)
   integer, intent(in out) :: A1(NA)
   integer, intent(in) ::B1(NB)
   integer, intent(in out) :: C1(NC)
...
recursive subroutine MergeSort(A,N,TEMP1,AT,TEMP2,ATT,TEMP3,ATTT,TEMP4,ATTTT,TEMP5,ATTTTT,TEMP6)

   integer, intent(in) :: N
   real*8, dimension(N), intent(in out) :: A
   real*8, dimension((N+1)/2), intent (out) :: TEMP1, TEMP2, TEMP3,TEMP4,TEMP5
   REAL*8, dimension(N), intent(in out) :: AT, ATT,ATTT,ATTTT
   integer, dimension(N), intent(in out) :: ATTTTT
   integer, dimension((N+1)/2), intent(in out) :: TEMP6
   ...
   call MergeSort(A,NA,TEMP1,AT,TEMP2,ATT,TEMP3,ATTT,TEMP4,ATTTT,TEMP5,ATTTTT,TEMP6)
   call MergeSort(A(NA+1),NB,TEMP1,AT(NA+1),TEMP2,ATT(NA+1),TEMP3,ATTT(NA+1),TEMP4,ATTTT(NA+1),TEMP5,ATTTTT(NA+1),TEMP6)
   ...
   if (A(NA) > A(NA+1)) then
      ...
      TEMP5(1:NA)=ATTTT(1:NA)
      TEMP6(1:NA)=ATTTTT(1:NA)
      call Merge(TEMP1,NA,A(NA+1),NB,A,N,TEMP2,AT(NA+1),AT,TEMP3,ATT(NA+1),ATT,TEMP4,ATTT(NA+1),ATTT,TEMP5,ATTTT(NA+1),ATTTT,TEMP6,ATTTTT(NA+1),ATTTTT)
   endif
   return
...
232         Z_1D(I)= cos(angle_beta)*(DV_1D_TOT + (0.5d0*(LX**2.0d0)*LZ*tan(angle_beta)))/(LX*LZ)
           ...
           IF((Z_1D(I)/(cos(angle_beta))) .LE. (GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))) THEN
 !           DO NOTHING
           ELSE
             I2 = I
...
246         Z_1D(I)= ABS(SQRT((LX*(GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))*sin(2*angle_beta))-DV_1D_TOT*sin(2*angle_beta)/LZ)) 
            Z_1D(I) = LX*sin(angle_beta) + (GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))*cos(angle_beta) - Z_1D(I)

           ELSE
            Z_1D(I) = LX*sin(angle_beta) + (GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))*cos(angle_beta)
           END IF
...
!         Z_STAR_1D(I)= 0.5d0*(Z_1D(I-1) + Z_1D(I))
         ...
         ELSE IF( I .LT. I2) THEN
            Z_STAR_1D(I)= 0.5d0*(Z_1D(I-1) + Z_1D(I))
        ELSE
          Z0 =  LX*sin(angle_beta) + (GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))*cos(angle_beta)
...
!!!!!!!!!!! CHECKING IF THERE IS ANY ANOMALY IN DRHO_DZSTAR !!!!!!!!!!
       IF(I .GT. 1) THEN
        ...
        IF(I .NE. NY_3D_TOT_TEMP) THEN
        DRHO_DZSTAR(i) = (TH_1D(I+1)-TH_1D(I-1))/(Z_STAR_1D(I+1)-Z_STAR_1D(I-1))
        ELSE
...
!       implicit none
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  CALCULATING THE BPE USING THE SORTED FIELD !!!!!!!!!!!!!!!!
       Z_1D_INITIAL(1:NY_3D_TOT_TEMP)=0.d0
       Z_STAR_1D_INITIAL(1:NY_3D_TOT_TEMP)=0.d0
       METHOD1 = .TRUE.
       METHOD2 = .FALSE. 
       METHOD3 = .FALSE. 
...
232         Z_1D_INITIAL(I)= cos(angle_beta)*(DV_1D_TOT + (0.5d0*(LX**2.0d0)*LZ*tan(angle_beta)))/(LX*LZ)
           ...
           IF((Z_1D_INITIAL(I)/(cos(angle_beta))) .LE. (GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))) THEN
 !           DO NOTHING
           ELSE
             I2 = I
!             write(*,*)'exceeded the dimension in z- direction'
           ...
           IF(I .NE. NY_3D_TOT_TEMP) THEN
246         Z_1D_INITIAL(I)= ABS(SQRT((LX*(GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))*sin(2*angle_beta))-DV_1D_TOT*sin(2*angle_beta)/LZ)) 
            Z_1D_INITIAL(I) = LX*sin(angle_beta) + (GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))*cos(angle_beta) - Z_1D_INITIAL(I)

           ELSE
            Z_1D_INITIAL(I) = LX*sin(angle_beta) + (GY(NY_TEMP_MAX+1)-GY(NY_TEMP_MIN))*cos(angle_beta)
           END IF
